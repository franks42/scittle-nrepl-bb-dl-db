...
(defn test-basic-operations []
  (try
    ...
    (catch Exception e
      (println "❌ Error during basic operations:" (.getMessage e))
      (println "Stack trace:")
      (.printStackTrace e)
      nil)))
...
(defn run-tests []
  ...
  (when (load-datalevin-pod!)
    (setup-datalevin-ns)
    
    ;; Run tests
    (when-let [conn (test-basic-operations)]
      (test-persistence)
      (check-database-files)
...
(def datalevin-config
  {:db-path db-path
   :schema schema
   :get-conn (fn [] (d/get-conn db-path schema))})

(comment
  ;; Example usage from REPL or other scripts:
  (require '[datalevin.core :as d])
  (def conn ((:get-conn datalevin-config)))
